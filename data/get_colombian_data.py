...
def get_species_to_collect():
    get_distributions_for_accepted_taxa(df.drop_duplicates(subset=[acc_name_col], keep='first'), acc_name_col,
...
def do_query(fulltext_dir, scrollID=None):
    ...
    search_string = '((title:phytochemistry OR title:phytochemical OR title:metabolomics) AND (title:colombia OR title:colombian))&_exists_:fullText'
    if scrollID is None:
...
def get_all_colombian_fulltexts():
    fulltext_dir = os.path.join(data_path, 'texts', f'colombian papers', 'fulltexts')
